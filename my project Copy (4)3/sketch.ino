...
#define BLYNK_AUTH_TOKEN "ODYk398JUTHSK1t_AHLrkGdSD-e42H9M"
...
const byte RELAY_FEEDBACK_PIN = 15;
...
void serviceCloudConnection(unsigned long now) {
  ...
  if (!Blynk.connected()) {
    cloudConnected = false;
    if (now - lastBlynkReconnectAt >= BLYNK_RECONNECT_INTERVAL_MS) {
  ...
  if (now - lastSignalQualityAt >= SIGNAL_QUALITY_INTERVAL_MS) {
    lastSignalQualityAt = now;
    int rssi = WiFi.RSSI();
    if (rssi < -75) {
      enqueueCloudEvent(CLOUD_ANOMALY, now);
    }
  }
...
if (safetyState == SAFETY_SAFE &&
    now - lastLcdPageAt >= LCD_PAGE_INTERVAL_MS) {
  lastLcdPageAt = now;
  lcdPage = (lcdPage + 1) % 6;
}
